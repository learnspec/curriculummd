...
**CurriculumMD** is an open, Markdown-based format for learning-objective frameworks — school syllabi, professional certification frameworks, internal training plans.

A `.curriculum.md` file is valid Markdown — readable by humans, versionable in Git, and consumable by any compatible LearnSpec player. It describes what a content corpus *must* cover, without being pedagogical content itself. It is the suite's first **meta** format: a reference framework against which a LearnSpec corpus can be assessed via AI alignment checks.
